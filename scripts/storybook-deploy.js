...
const DEV_PATH =
  ...
  (process.env.IX_DEV ? `${process.env.IX_DEV}/${PROJECT_SLUG}` : process.cwd());
...
function deploy() {
  ...
  if (WATCH_POLLING === 'true') {
    ...
    console.log(
      `${GRAY}💡 If changes aren't detected, try: WATCH_POLLING=true pnpm run storybook:deploy${RESET}`
    );
  }
  console.log('');
}
...
function health() {
  ...
  const httpCode = execQuiet(`curl -s -o /dev/null -w "%{http_code}" ${APP_URL}`, {
    fallback: '000',
  });
  if (httpCode === '200') {
